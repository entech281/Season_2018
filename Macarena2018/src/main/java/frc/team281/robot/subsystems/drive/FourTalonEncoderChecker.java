...
public class FourTalonEncoderChecker {
	...
	public void updateMotorsToWorkAroundBrokenEncodersIfEnabled(FourTalonGroup talons,
			DriveEncoderStatus driveEncoderStatus) {
		if (!workAroundBrokenEncoders)
			return;

		// set these directly on the talons for immediate results.
		// another way would be to change the settings and then apply them to the
		// talons,
