...
/**
 * This class holds talon settings.
 * Sometimes you need to re-program a Talon
 * on the fly when you're changing modes,
 * so this allows you to keep a set of configurations
 * in memory and continually re-apply them. ( vs the lazy way of just setting them
 * in the talon and forgetting about it )
 * 
...
public class TalonSettings implements Serializable{
	...
	/**
	 * This class doesnt use javabean conventions
	 * becasue its just a huge structure,
	 * and because we use TalonBuilder to enforce all of the 
	 * requirements ( IE, get/set is not sufficient to enforce 
	 * the semantics of this very complex setup process for a Talon
	 */
	...
	public void configureTalon(TalonSRX talon) {
        ...
        talon.setStatusFramePeriod(StatusFrameEnhanced.Status_13_Base_PIDF0, this.framePeriods.pidMilliseconds, TIMEOUT_MS);
        talon.setStatusFramePeriod(StatusFrameEnhanced.Status_10_MotionMagic, this.framePeriods.motionMagicMilliseconds, TIMEOUT_MS);
	...
	/**
	 * Sets just the mode.
	 * Typically used when you're swtiching back and forth
	 * between modes
	 * @param talon
	 */
